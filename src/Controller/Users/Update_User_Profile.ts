    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await UserRepo.save(existingUser)
                .then((data) => {
                    return RoutesHandler.sendSuccess(res, req, data, 'User updated successfully');
                })
                .catch((err) => {
                    console.log(err);
                    return RoutesHandler.sendError(res, req, 'Failed to update User', ResponseCodes.saveError);
                });
